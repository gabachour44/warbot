...
ring_table = db.table('ring_table')
...
async def command_pcw(message):
    ...
    #Broadcast
    for settings in guildList:
        if not settings['muted']:
            guild =  next((elem for elem in client.guilds if elem.id == settings['id']), None)
            if not guild:
                continue
...
async def command_activate(message):
    ...
    if guild_table.search(Query().id == message.guild.id):
        guild_table.update({'channel': message.channel.id}, Query().id == message.guild.id)
    else:
...
async def on_message(message):
    ...
    if message.content.startswith( botPrefix + 'warbot on' ):
        await command_activate(message)
        return

    if message.content.startswith( botPrefix + 'warbot help' ):
        await command_help(message)
        return

    if message.content.startswith( botPrefix + 'warbot about' ):
        await command_about(message)
        return

    if not guild_table.search(Query().id == message.guild.id):
        return

    settings = guild_table.search(Query().id == message.guild.id)
    if message.channel.id != settings[0]['channel']:
        return

    if message.content.startswith( botPrefix + 'warbot mute'):
        await command_mute(message)
        return

    if message.content.startswith( botPrefix + 'warbot unmute'):
        await command_unmute(message)
        return

    if message.content.startswith( botPrefix + 'warbot list'):
